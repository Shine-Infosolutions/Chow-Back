...
} = require("../config/gorakhpurPincodes");
...
const updateStock = async (items = []) => {
  for (const i of items) {
    ...
    await Item.findByIdAndUpdate(i.itemId, { $inc: { stockQty: -i.quantity } });
  }
};
...
exports.createOrder = async (req, res) => {
  try {
    ...
    const deliveryInfo = await deliveryService.getDeliveryInfo(
    ...
    deliveryService.validateDeliveryProvider(
      orderData.deliveryPincode,
      deliveryProvider,
    );
    ...
    const platformFee = Math.ceil(
      ((subtotal + gstAmount + shippingTotal) * PLATFORM_FEE_PERCENT) / 100,
    );
    const finalAmountInRupees =
      Math.round((subtotal + gstAmount + shippingTotal + platformFee) * 100) /
      100;
    ...
    if (finalAmountInRupees > 100000) {
      throw new Error(
        "Order amount seems too high - possible currency conversion error",
      );
    }
    ...
    const orderDoc = await Order.create({
      ...
      shipping: {
        ...
        breakdown: deliveryInfo.breakdown || {
          [deliveryProvider]: shippingTotal,
        },
        eta: deliveryETA,
      },
      totalWeight,
...
exports.razorpayWebhook = async (req, res) => {
  try {
    ...
    if (!order) {
      ...
      return res
        .status(200)
        .json({ received: true, message: "Order not found" });
    }

    console.log(
      `Found order ${order._id} with status: ${order.status}, paymentStatus: ${order.paymentStatus}`,
    );
    ...
    switch (event) {
      ...
      case "payment.authorized":
        console.log(
          "Payment authorized but not captured yet for order:",
          order._id,
        );
        order.razorpayData.push({
...
exports.confirmPayment = async (req, res) => {
  try {
    ...
    if (!paymentData) {
      return res
        .status(400)
        .json({ success: false, message: "No payment ID found" });
    }
...
exports.cleanFailedOrders = async (req, res) => {
  try {
    ...
    const ordersToDelete =
      await Order.find(failedOrdersQuery).populate("items.itemId");
    ...
    for (const order of ordersToDelete) {
      if (order.items && order.items.length > 0) {
        for (const item of order.items) {
          if (item.itemId) {
            await Item.findByIdAndUpdate(item.itemId._id, {
